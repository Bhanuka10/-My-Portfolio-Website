const projectsData = [
  {
    ...
    prompt:"Leadership & Coordination: Took the lead role in guiding the team, managing tasks, and ensuring smooth communication and collaboration throughout the project. I coordinated work distribution, helped resolve technical challenges, and ensured that deadlines were met.UI/UX Design: Designed the complete interface in Figma, creating a user-friendly and visually consistent application.AI Integration: Implemented the AI-driven recommendation system that analyzes user profiles to suggest personalized learning resources.Roadmap Generation: Developed the roadmap generator that provides structured learning paths from beginner to expert level.YouTube API Integration: Integrated the YouTube Data API to fetch, filter, and display educational playlists based on relevance, popularity, and user needs.Chatbot Development: Created the chatbot feature to extract user goals and recommend suitable learning resources interactively.Full-Stack Development: Worked across both frontend and backend to ensure seamless integration of all modules.",
